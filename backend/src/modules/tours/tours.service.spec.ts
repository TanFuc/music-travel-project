...
describe('ToursService', () => {
    ...
    describe('create Combo vs Tour', () => {
        it('creates a normal tour properly (isCombo=false)', async () => {
            ...
            const result = await service.create(createDto as any, undefined);
            expect(prisma.tour.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('findAll', () => {
        it('returns split data based on isCombo flag', async () => {
            ...
            const result = await service.findAll({ page: 1, limit: 10, isCombo: true });
