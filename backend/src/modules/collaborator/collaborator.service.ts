...
const MAX_VOUCHER_DISCOUNT = 100000; // 100k VND max commission per ticket
const BASE_COMMISSION_PER_TICKET = 100000; // 100k VND base
...
export class CollaboratorService {
  ...
  // Update collaborator content (admin only)
  ...
  // Create a voucher for collaborator
  async createVoucher(userId: number, dto: CreateCollaboratorVoucherDto) {
    // Validate that user is a collaborator
    ...
    // Validate discount value - max commission is 100k
    ...
    // For percentage discounts, ensure maxDiscountAmount is set and capped
  ...
  // Get commission summary for collaborator
  async getCommissionSummary(userId: number): Promise<CommissionSummary> {
    const wallet = await this.prisma.userWallet.findUnique({
      where: { userId },
    });

    // Get all bookings that used collaborator's vouchers
    ...
    for (const booking of bookingsWithCommission) {
      ...
      // Commission = (100k * ticketQty) - discountGiven
    ...
    // Get pending commissions (unpaid bookings)
  ...
  async getCommissionDetails(
    ...
    const details: CommissionDetail[] = bookings.map((booking) => {
      ...
      const maskedName =
        fullName.length > 2 ? fullName[0] + '*'.repeat(fullName.length - 2) + fullName[fullName.length - 1] : fullName;
  ...
  // Process commission when order is paid (called from payment service)
  async processCommission(bookingId: number) {
    ...
    if (!booking || !booking.voucher || !booking.voucher.ownerId) {
      return null; // No voucher or not a collaborator voucher
    }
    ...
    // Add commission to collaborator's wallet
  ...
  // Become a collaborator (user request)
