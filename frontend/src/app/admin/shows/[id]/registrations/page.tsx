...
import { Check, X, Clock, Mic2, User } from 'lucide-react';
...
export default function ShowRegistrationsPage({
  ...
  const reviewMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      patch(`/admin/performance/registrations/${id}/review`, {
        status,
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="py-12 text-center text-neutral-500">Chưa có đăng ký nào.</div>
          ) : (
            <div className="space-y-4">
              {registrations.map((reg) => (
                <div
                  key={reg.id}
                  className="flex flex-col items-start justify-between gap-4 rounded-lg border p-4 transition-colors hover:bg-neutral-50 md:flex-row md:items-center"
                >
                  <div className="flex-1">
                    <div className="mb-2 flex items-center gap-2">
                      <h3 className="text-lg font-bold">{reg.songTitle}</h3>
                      <Badge variant={getStatusColor(reg.status)}>{reg.status}</Badge>
                    ...
                    <div className="grid grid-cols-1 gap-x-6 gap-y-1 text-sm text-neutral-600 sm:grid-cols-2">
                      <div className="flex items-center gap-2">
                        <User className="h-4 w-4" />
                        <span className="font-medium">{reg.guestName}</span>
                        {reg.guestPhone && (
                          <span className="text-neutral-400">({reg.guestPhone})</span>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
