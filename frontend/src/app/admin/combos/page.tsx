...
import { Calendar, Trash, Edit, Star } from 'lucide-react';
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {data.items.map((tour: any) => (
                <div
                  key={tour.id}
                  className="flex flex-col justify-between rounded-lg border bg-amber-50 p-4 hover:shadow-md"
                >
                  <div>
                    <h3 className="mb-2 line-clamp-2 text-lg font-semibold text-amber-900">
                      {tour.title}
                    </h3>
                    <p className="text-sm font-semibold text-amber-700">
                      Linked Show ID: {tour.linkedShowId || 'Chưa Link'}
                    </p>
