...
export interface QRPaymentResponse {
  ...
  /** VietQR.io image URL - use this for QR accepted by all bank apps */
  qrImageUrl?: string;
  qrContent: string;
  bank: {
    bankCode: string;
...
class PaymentService {
  ...
  async healthCheck(): Promise<{ message: string; supportedBanks: number }> {
  ...
  openBankApp(deeplink: string): void {
    ...
    // Fallback: detect if on mobile and show instructions
    if (this.isMobile()) {
      setTimeout(() => {
        if (!document.hidden) {
          // If still on page after 2 seconds, likely deeplink failed
