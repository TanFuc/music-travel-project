...
import { QrCode, User, Music, AlertCircle, CheckCircle2 } from 'lucide-react';
        ...
        <div className="space-y-6">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              <QrCode className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" />
              <Input
              ...
              <CardContent className="space-y-4 p-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-1">
                    <p className="flex items-center gap-1 text-xs font-medium uppercase text-neutral-500">
                  ...
                  <div className="space-y-1 text-right">
                    <p className="flex items-center justify-end gap-1 text-xs font-medium uppercase text-neutral-500">
                      <User className="h-3 w-3" /> Người sở hữu
                    </p>
                    <p className="font-bold">{ticket.booking?.user?.fullName || 'Vé vãng lai'}</p>
                  ...
                  {ticket.status === 'USED' ? (
                    <div className="flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-4 text-green-800">
                      ...
                      <div>
                        ...
                        <p className="text-sm">
                          Tại show:{' '}
                          <strong>{ticket.redeemedShow?.title || 'Không xác định'}</strong>
                        </p>
                        <p className="text-sm italic">
                          Thời gian: {formatDate(ticket.checkedInAt)}
                        </p>
                      </div>
                    </div>
